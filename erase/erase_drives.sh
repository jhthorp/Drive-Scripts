#!/bin/bash
################################################################################
#                             TrueNAS Drives Erase                             #
#                                                                              #
#             This script will export a single function that will              #
#                begin a drive erase for a collection of drives                #
################################################################################
...
################################################################################
#===============================================================================
# This function will begin a drive erase for a collection of drives.
#
# The following steps will be performed:
#   01) Start a new TMUX session named “drive_erase[_{session_suffix}]” 
...
#   04) Setup a TMUX grid layout for all drives being erased
#   05) Attach to the new TMUX session
#   06) Start the Erase process
...
#
# ARGUMENTS:
#   [1 - drives_override] The array of drive IDs to erase
#   [2 - session_suffix] The suffix to add to the session name
#   [3 - end_on_detach] The switch to end the process when the TMUX session is 
#                       detached
#
# OUTPUTS:
#   N/A - N/A
...
erase_drives ()
{
  ...
  $(send_tmux_pane_command \
    0 \
    "echo Beginning process to erase drives..." \
  )
  ...
  # Print the drives being erased
  echo "Available Drives To Erase: [${available_drives[@]}]"
  ...
  then
    ...
    do
      SELECTED=false
      read -p "Would you like to erase drive: ${drive} (y/n)?" choice
      case "$choice" in 
        y|Y ) SELECTED=true;;
        n|N ) SELECTED=false;;
        * ) echo "Invalid Entry";;
      ...
      if [ "${SELECTED}" = true ]
      then
        drives_to_erase+=(${drive})
      fi
      unset SELECTED
    done

    # Print the drives being erased
    echo "Selected Drives To Erase: [${drives_to_erase[@]}]"
    ...
    if [ "${CONTINUE}" = true ]
    then
      filtered_drives_to_erase=("${drives_to_erase[@]}")
    fi
  else
    filtered_drives_to_erase=("${available_drives[@]}")
  fi
  ...
  then
      ...
      # Loop over each selected drive and begin the erase process
      PANE_ITER=1
      for drive in "${filtered_drives_to_erase[@]}"
      do
        echo "Preparing to Erase drive: ${drive}"

        # Log the drive activity
        $(send_tmux_pane_command \
          ${PANE_ITER} \
          "echo Erasing drive: ${drive}" \
        )
        ...
        $(send_tmux_pane_command \
          ${PANE_ITER} \
          "bash ${ERASE_SCRIPT} ${AUTOMATED_SKIP} ${drive}" \
        )
